...
class QLearning:
    ...
    def step(self, learning_rate, discount_rate):
        ...
        if flag:
            self.Q_table[self.state, action] += self.reward(next_state, flag)
        else:    
            self.Q_table[self.state, action] += learning_rate * (
            self.reward(next_state, flag)
            + discount_rate * self.Q_table[next_state, next_action]
            - self.Q_table[self.state, action]+0.1*self.huristics())
...
for epoc in range (epocs):
    ...
    np.append(table,[step])
    total_num+=step/1000

print(total_num)

x=np.arrange(len(table))
plt.figure(figsize=(8, 4))
plt.scatter(x, table, color='blue', label="Values")
plt.xlabel("Index")
plt.ylabel("Value")
plt.title("Dot Plot of NumPy Array")
